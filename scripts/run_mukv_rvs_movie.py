...
class MuKVRVSMovieRunner(BaseVQA):
    """MuKV RVS-Movie 入口。"""
    ...
    def save_memory_stats(self):
        ...
        with open(memory_stats_path, 'w', newline='', encoding='utf-8') as f:
            fieldnames = [
                'video_id', 'timestamp', 'gpu_memory_mb', 'kv_cache_mb',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='MuKV on RVS-Movie')
    
    # 基础参数
    parser.add_argument("--model_path", type=str, default="llava-hf/llava-onevision-qwen2-0.5b-ov-hf")
    parser.add_argument("--anno_path", type=str, default="data/rvs/movie/movienet_oe.json")
    ...
    parser.add_argument("--save_dir", type=str, default="results/mukv/rvs_movie")
    ...
    logger.info("="*80)
    logger.info("MuKV inference on RVS-Movie")
    logger.info("="*80)
