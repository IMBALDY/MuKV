...
from model import llava_onevision_mukv


MODELS = {
    'llava_ov_0.5b': {
        'load_func': llava_onevision_mukv.load_model,
    ...
    'llava_ov_7b': {
        ...
        'model_class': LlavaOnevisionForConditionalGeneration,
        'processor_class': LlavaOnevisionProcessor,
        'model_path': 'model_zoo/llava_onevisionqwen2_7b_ov_hf',
    },
    'llava_ov_72b': {
        'load_func': llava_onevision_mukv.load_model,
        'model_class': LlavaOnevisionForConditionalGeneration,
...
class BaseVQA:
    ...
    def split_list(self, lst, n):
        """Split a list into n non-overlapping chunks"""
        total_len = len(lst)
        chunk_size = total_len // n  # 整除，确保不重叠
        remainder = total_len % n     # 余数分配给前几个chunk
        
        chunks = []
        start = 0
        for i in range(n):
            # 前remainder个chunk多分配一个元素
    ...
    def get_chunk(self, lst, n, k):
        ...
        logger.info(f"Chunk {k}/{n}: processing {len(chunks[k])} videos (total: {len(lst)})")
        return chunks[k]
