...
class GPTService:
    ...
    def gpt_with_retry(self, prompt, max_retries=10):
        """重试机制，最多重试10次"""
        for attempt in range(max_retries):
            try:
                ...
                if attempt % 5 == 0:  # 每5次打印一次
                    print(f"Retry {attempt+1}/{max_retries}: {e}")
            
            # 指数退避：失败次数越多，等待越久
            wait_time = min(2 ** (attempt // 5), 15)  # 最多等15秒
    ...
    for file in tqdm(caption_files):
        ...
        try:
            ...
            if response_message is None:
                print(f"⚠️ Skipping {key} - API连续失败，跳过不处理")
                continue  # 直接跳过，不保存任何结果
            
            # Convert response to a Python dictionary.
            try:
                response_dict = ast.literal_eval(response_message)
            except (ValueError, SyntaxError) as parse_err:
                print(f"⚠️ Parse error for {key}: {parse_err}, response: {response_message}")
                # 解析失败也跳过
                continue
...
def main():
    ...
    if 'retrieve_size' in pred_contents[0]:  # NOTE: change this line if experiment with different hyper-parameters
        pred_contents = [x for x in pred_contents if (x['retrieve_size']==64 or x['retrieve_size']==128) and x['chunk_size']==1]
